...
function M.setup(user_config)
	...
	vim.api.nvim_create_user_command("QuickFailReload", function()
		-- Easy Reloading
		package.loaded["quickfail"] = nil
		require("quickfail").setup()
	end, {})

	--- use like `vim.cmd[[ReRunCmd]]`
	vim.api.nvim_create_user_command(
		...
		function(args)
			...
			-- local cmd = vim.split(cmd_str, "%s+", {})
			--
			-- cmd_str = cmd_str:match("^%s*(.*)")
			-- vim.print((" a       b   "):match("%s*(.*)"))
			-- vim.print((" a       b   "):gsub("%s+", " "))
			-- -- vim.print(vim.split(cmd_str, "%s+",{}))
			local cmd = vim.split(cmd_str, "%s+", {})
			...
			--TODO: unpack?
			logic.rerecmd(pattern, cmd, keycodes)
		end,
		{
	...
	vim.api.nvim_create_user_command(
		...
		function(args)
			...
			for k, _ in pairs(options) do
				table.insert(option_keys, k)
			end

			vim.ui.select(option_keys, { prompt = "Choose:" }, function(choice)
				print("selected:", choice)
				-- vim.print()

				local keycodes = "<F13>"
				local pattern = "*"
				local cmd = options[choice].cmd
